  ...
  xwalk_loop: do ixw = 1,num_xw
     ...
     if (all_continue) then
        ...
        case('tauu','tauv','hfss','rlut','rlutcs','hfls','rlus','rsus','rsuscs','rsut','rsutcs','mc',&
             ...
             'emico','emiisop','emibisop','eminh3','emipom','emivoc','eminox')
           mycmor%positive = 'up'
...
!           mycmor%positive = 'down'
        ...
        case ('acceldiv','accelgw','accelnogw','accelogw')
...
!
        ...
        !
        select case (xw(ixw)%entry)
        case ('aoa','aoa_nh','airmass','chegpso4','cl','cli','clw',&
             'do3chm','emilnox','jno2','lossch4','lossco',&
             ...
             'losso1dviah2o','losso3viaho2','losso3viaoh','lossrcoo2viano2','lossro2viaho2',&
              ...
              'mmraernh4','mmraerno3','mmraerso4','acceldiv','accelgw','accelnogw','accelogw')
        ...
        !
        select case (xw(ixw)%entry)
        case ('clt','cod','od550aer','od550bc','od550oa','ps',&
        ...
        case ('emipom')
           ...
           enddo
        case ('od550dust','od550ss','emidust')
           !
           ! Sum four fields
           !
        ...
        case ('emivoc','emibvoc')
           ...
           do ic = 1,nchunks(1)
              do it = tidx1(ic),tidx2(ic)
                 ...
                 call read_var(myncid(1,19),var_info(var_found(1,19))%name,indat2_19)
                 !
                 where ((indat2_01 /= 1.e36).and.(indat2_02 /= 1.e36).and.(indat2_03 /= 1.e36).and.(indat2_04 /= 1.e36).and.&
                        ...
                        (indat2_17 /= 1.e36).and.(indat2_18 /= 1.e36).and.(indat2_19 /= 1.e36))
                   ...
                   indat2_19 = indat2_19 * (12./ 27.)

                   cmordat2d = indat2_01+indat2_02+indat2_03+indat2_04+indat2_05+indat2_06+indat2_07+indat2_08+indat2_09+indat2_10+indat2_11+indat2_12+indat2_13+indat2_14+indat2_15+indat2_16+indat2_17+indat2_18+indat2_19
                 elsewhere
                    cmordat2d = spval
                 endwhere
                 !
...
!                    write(*,*) minval(cmordat3d),maxval(cmordat3d,mask=cmordat3d/=spval),minval(zonave),maxval(zonave,mask=zonave/=spval)
              ...
              enddo
           enddo
        case ('aoa','aoa_nh','cl','cli','clw','hus','jno2','mcu','photo1d','pilev','pmlev',&
              ...
              'mmraernh4','mmraerno3','mmraerso4','acceldiv','accelogw','dh')
           !
        ...
        case ('airmass','do3chm','lso3chm','tpo3chm')
           !
           ! Just one field, leave on model levels
           !
           allocate(indat3a(nlons,nlats,nlevs))
           ...
           do ifile = 1,nc_nfiles(1)
              ...
              do ic = 1,nchunks(ifile)
                 do it = tidx1(ic),tidx2(ic)
                    ...
                    cmordat3d = indat3a*mw_so4/1000./avogn * psdelta/grav/rho
                    !
                    tval(1) = time(it) ; tbnd(ifile,1) = time_bnds(1,it) ; tbnd(2,1) = time_bnds(2,it)
                    error_flag = cmor_write(        &
                         var_id        = cmor_var_id,   &
            ...
            do ifile = 1,nc_nfiles(1)
              ...
              do ic = 1,nchunks(ifile)
                 do it = tidx1(ic),tidx2(ic)
                    ...
                    rho = pshybrid_mid/(287.04*tdata)
                    !
                    work3da = (indat3a*(psdelta/grav))/rho/1000./avogn
                    cmordat2d = sum(work3da,dim=3)
                    !
                    tval(1) = time(it) ; tbnd(ifile,1) = time_bnds(1,it) ; tbnd(2,1) = time_bnds(2,it)
                    error_flag = cmor_write(        &
        ...
        case ('accelgw')
           !
           ! Sum three fields, leave on model levels
           !
           allocate(indat3a(nlons,nlats,nlevs),indat3b(nlons,nlats,nlevs),indat3c(nlons,nlats,nlevs))
           allocate(psdata(nlons,nlats))
           allocate(cmordat3d(nlons,nlats,nlevs))
           ...
           do ifile = 1,nc_nfiles(1)
              ...
              do ic = 1,nchunks(ifile)
                 do it = tidx1(ic),tidx2(ic)
                    ...
                    call read_var(myncid(ifile,1),var_info(var_found(ifile,1))%name,indat3a)
                    call read_var(myncid(ifile,2),var_info(var_found(ifile,2))%name,indat3b)
                    call read_var(myncid(ifile,3),var_info(var_found(ifile,3))%name,indat3b)
