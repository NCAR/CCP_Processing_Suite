MODULE popregrid
  ...
  !-----------------------------------------------------------------------------
  !   CVS:$Id: ccsm_ocn_rgd.F90,v 1.2 2005/03/31 17:38:18 strandwg Exp $
  !-----------------------------------------------------------------------------
  ...
  INTEGER(KIND=int_kind)::map_ncid,src_ncid
  INTEGER(KIND=int_kind)::grid_ncid, grid_varid
  INTEGER(KIND=int_kind)::src_imt, src_jmt, timelen, unlimdimid, status
  INTEGER(KIND=int_kind)::src_grid_size,dst_grid_size,num_links
  INTEGER(KIND=int_kind)::idum1,idum2,idum3

  CHARACTER(LEN=char_len),DIMENSION(max_vars):: src_depth_name

  CHARACTER(LEN=char_len):: src_var_units,src_var_lname, &
       src_depth_edges_name, src_time_name, &
  ...
  REAL(KIND=real_kind), DIMENSION(:), ALLOCATABLE :: &
       src_lon,src_lat,time

  REAL(KIND=real_kind), DIMENSION(:,:), ALLOCATABLE :: src_depth
...
CONTAINS
  ...
  SUBROUTINE read_src_file_info
    ...
    INTEGER(KIND=int_kind) :: ndims,gndims,gvarid,varid,ivar,i
    ...
    DO ivar = 1,nvars
       IF (src_var(ivar)=='TAUX' .or.src_var(ivar)=='TAUY' .or. &
           src_var(ivar)=='SU'   .or.src_var(ivar)=='SV'   .or. &
           src_var(ivar)=='UVEL' .or.src_var(ivar)=='VVEL' .or. &
           src_var(ivar)=='UISOP'.or.src_var(ivar)=='VISOP'.or. &
           src_var(ivar)=='UES'  .or.src_var(ivar)=='VNS'  .or. &
           src_var(ivar)=='UET'  .or.src_var(ivar)=='VNT') THEN
    ...
    DO ivar = 1,nvars
       ...
       IF (ndims > 2) THEN
          CALL nf_inq_dim_wrap(src_ncid, dimids(3), src_depth_name(ivar), src_km(ivar))

          IF (INDEX(upper(src_depth_name(ivar)), 'Z') /= 1) THEN
             IF (((INDEX(upper(src_depth_name(ivar)), 'TIME') == 1) .OR. &
                  (INDEX(upper(src_depth_name(ivar)), 'DATE') == 1) .OR. &
                  (INDEX(upper(src_depth_name(ivar)), 'T') == 1) .OR. &
                  (INDEX(upper(src_depth_name(ivar)), 'MONTH') == 1)) .AND. &
                  (ndims == 3)) THEN
                src_time_name = src_depth_name(ivar)
                src_depth_name(ivar) = 'unknown'
                timelen = src_km(ivar)
                src_km(ivar) = 0
             ELSE
                CALL msg_write(sub_name, 'unexpected src_depth_name ', &
                     TRIM(src_depth_name(ivar)))
                STOP
             END IF
          END IF
    ...
    !  get POP grid
    ALLOCATE(ANGLE(src_imt,src_jmt))
    if ((src_var(ivar) == 'UVEL').or.(src_var(ivar) == 'VVEL')) then
       CALL nf_inq_varid_wrap(src_ncid, 'ANGLE', varid)
    else
       CALL nf_inq_varid_wrap(src_ncid, 'ANGLET', varid)
    endif
    CALL nf_get_var_wrap(src_ncid, varid, ANGLE)

    DO ivar = 1,nvars
       IF (src_km(ivar) > 0) THEN
          IF (.NOT.ALLOCATED(src_depth)) ALLOCATE(src_depth(src_km(ivar),max_vars))
          if (src_var(ivar) == 'WVEL') then
             CALL nf_inq_varid_wrap(src_ncid, 'z_w', varid)
          else
             CALL nf_inq_varid_wrap(src_ncid, 'z_t', varid)
          endif
          CALL nf_get_var_wrap(src_ncid, varid, src_depth(:,ivar))
          CALL nf_get_att_wrap(src_ncid, varid, 'units', zunits)
          IF(INDEX(upper(zunits), 'CENTI') /= 0)  &
               src_depth(:,ivar)=src_depth(:,ivar)/100.
       END IF
...
!       IF (dst_IMASK_varid > 0) dst_needs_IMASK = .FALSE.
  ...
  SUBROUTINE def_dst_file(ivar)
    ...
    IF (dst_needs_depth) THEN
       dst_depth_name = src_depth_name(ivar)
...
!!$    IF (dst_needs_AREA) THEN
!!$       dst_AREA_name = 'AREA'
!!$       CALL nf_def_var_wrap(dst_ncid(ivar), dst_AREA_name, NF_FLOAT, 2, &
!!$            (/ dst_X_dimid, dst_Y_dimid /), dst_AREA_varid)
!!$
!!$       att_name = 'units'
!!$       att_string = 'centimeter^2'
!!$       CALL nf_put_att_wrap(dst_ncid(ivar), dst_AREA_varid, att_name, &
!!$            LEN_TRIM(att_string), att_string)
!!$
!!$       att_name = 'long_name'
!!$       att_string = 'area of grid cells'
!!$       CALL nf_put_att_wrap(dst_ncid(ivar), dst_AREA_varid, att_name, &
!!$            LEN_TRIM(att_string), att_string)
!!$
!!$    END IF
!!$
!!$    IF (dst_needs_IMASK) THEN
!!$       dst_IMASK_name = 'IMASK'
!!$       CALL nf_def_var_wrap(dst_ncid(ivar), dst_IMASK_name, NF_INT, 2, &
!!$            (/ dst_X_dimid, dst_Y_dimid /), dst_IMASK_varid)
!!$
!!$       att_name = 'long_name'
!!$       att_string = 'Binary grid mask'
!!$       CALL nf_put_att_wrap(dst_ncid(ivar), dst_IMASK_varid, att_name, &
!!$            LEN_TRIM(att_string), att_string)
!!$
!!$    END IF
  ...
  SUBROUTINE put_aux_vars(ivar)
    ...
    IF (dst_needs_LAT) THEN
       IF (.NOT.ALLOCATED(LAT2D)) ALLOCATE(LAT2D(dst_imt,dst_jmt))
       LAT2D = reshape(dst_grid_center_lat,(/dst_imt,dst_jmt/))
!       CALL nf_put_var_wrap(dst_ncid(ivar), dst_LAT_varid, LAT2D)
    ...
    IF (dst_needs_LON) THEN
       IF (.NOT.ALLOCATED(LON2D)) ALLOCATE(LON2D(dst_imt,dst_jmt))
       LON2D = reshape(dst_grid_center_lon,(/dst_imt,dst_jmt/))
!       CALL nf_put_var_wrap(dst_ncid(ivar), dst_LON_varid, LON2D)
    ...
    IF (dst_needs_AREA) THEN
       IF (.NOT.ALLOCATED(AREA)) ALLOCATE(AREA(dst_imt,dst_jmt))
       AREA = reshape(dst_grid_area, (/dst_imt,dst_jmt/))
       AREA(:,:) = AREA(:,:)*radius*radius
       IF (MAXVAL(AREA)==0.0) CALL compute_area(LAT,LON,AREA)
!       CALL nf_put_var_wrap(dst_ncid(ivar), dst_AREA_varid, AREA)
    END IF

    IF (dst_needs_IMASK) THEN
       IF (.NOT.ALLOCATED(IMASK)) ALLOCATE(IMASK(dst_imt,dst_jmt))
       IMASK = reshape(dst_grid_imask, (/dst_imt,dst_jmt/))
!       CALL nf_put_var_wrap(dst_ncid(ivar), dst_IMASK_varid, IMASK)
    END IF
...
!    WRITE(*,*) 'Entering ',trim(sub_name),' ',trim(src_var(ivar)),' ',k,l
    ...
    IF (dst_z_standard) THEN
       IF ((dst_kmin /= 0).AND.(dst_kmax /= 0)) THEN
          DO dst_lev = dst_kmin,dst_kmax
             ...
             src_lev = MINLOC(ABS(src_depth(:,ivar)-dst_z), DIM=1)
             src_z = src_depth(src_lev,ivar)
             !         WRITE(*,*) 'REGRID_WRAP at LEVEL:',dst_lev,src_lev,src_z,dst_z
             
             IF (equal(src_z, dst_z, RELTOL=1.0e-4)) THEN
                !            WRITE(*,*) 'REGRID_WRAP no change at: ',src_z,dst_z
                CALL nf_get_vara_wrap(src_ncid, src_varid(ivar), (/ 1, 1, src_lev, l /), &
                     (/ src_imt, src_jmt, 1, 1 /), src_data)
                IF (src_var_rotate(ivar)) THEN
                   CALL nf_get_vara_wrap(src_ncid, src_varid2(ivar), (/ 1, 1, src_lev, l /), &
                        (/ src_imt, src_jmt, 1, 1 /), src_data2)
                   CALL rotate(msv,src_data,src_data2,ivar)
                END IF
                CALL read_map_file(src_lev)
                CALL regrid_level(msv, src_data, src_data_above_on_dst,ivar)
                src_data_below_on_dst = src_data_above_on_dst
                wz = c0
             ELSE
                src_lev = MAXLOC(src_depth(:,ivar), DIM=1, MASK=(src_depth(:,ivar)<=dst_z))
                if (src_lev           == 0) src_lev = 1
                if (src_lev > src_km(ivar)) src_lev = src_km(ivar)
                !            write(*,*) 'REGRID_WRAP: src_lev ',src_lev
                src_z_above = src_depth(src_lev,ivar)
                ...
                src_lev = MINLOC(src_depth(:,ivar), DIM=1, MASK=(src_depth(:,ivar)>dst_z))
                if (src_lev           == 0) src_lev = 1
                if (src_lev > src_km(ivar)) src_lev = src_km(ivar)
                !            write(*,*) 'REGRID_WRAP: src_lev ',src_lev
                src_z_below = src_depth(src_lev,ivar)
                ...
                END IF
                CALL read_map_file(src_lev)
                CALL regrid_level(msv, src_data, src_data_below_on_dst,ivar)
                
                IF (abs(src_z_below - src_z_above) < 0.001) THEN
                   wz = c0
                   !               WRITE(*,*) 'REGRID_WRAP 0 wz: ',wz,dst_z,src_z_above,src_z_below
                ELSE
                   wz = (dst_z - src_z_above) / (src_z_below - src_z_above)
                   !               WRITE(*,*) 'REGRID_WRAP 1 wz: ',wz,dst_z,src_z_above,src_z_below
                ENDIF
             END IF
             ...
             DO j = 1,dst_jmt
                ...
                END DO
             END DO
             CALL nf_put_vara_wrap(dst_ncid(ivar), dst_varid(ivar), (/ 1, 1, dst_lev, l /), &
       ...
       ELSE
          ...
          IF (dst_kmin == 0) THEN
             ...
             IF (src_var_rotate(ivar)) THEN
                ...
                CALL rotate(msv,src_data,src_data2,ivar)
             END IF
             CALL regrid_level(msv, src_data, dst_data,ivar)
    ...
    ELSE
       ...
       IF (dst_kmin == 0) THEN
          ...
          RETURN
       END IF
       !   handle case where src vertical axis has nk levels
       IF (src_km(ivar) >= 1) THEN
          CALL nf_get_vara_wrap(src_ncid, src_varid(ivar), (/ 1, 1, k, l /), &
               (/ src_imt, src_jmt, 1, 1 /), src_data)
          IF (src_var_rotate(ivar)) THEN
             CALL nf_get_vara_wrap(src_ncid, src_varid2(ivar), (/ 1, 1, k, l /), &
                  (/ src_imt, src_jmt, 1, 1 /), src_data2)
             CALL rotate(msv,src_data,src_data2,ivar)
          END IF

          CALL regrid_level(msv, src_data, dst_data,ivar)
          CALL nf_put_vara_wrap(dst_ncid(ivar), dst_varid(ivar), (/ 1, 1, k, l /), &
            (/ dst_imt, dst_jmt, 1, 1 /), dst_data)
          RETURN
       END IF
    END IF
  ...
  SUBROUTINE regrid_level(msv, src_data, dst_data, ivar)
...
!    IF (.NOT.src_var_tgrid(ivar)) CALL regrid_to_tgrid(msv,src_data)
